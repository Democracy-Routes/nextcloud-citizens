	...
	<div class="rc-fill">
		...
		<div class="rc-scroll">
			...
			<template v-else-if="report">
				...
				<div v-for="round in report.rounds" :key="round.position" class="rc-card">
					...
					<template v-for="group in groupByType(round.cross_table)" :key="group.type">
						...
						<div v-for="finding in group.findings" :key="finding.id" style="margin: 0 0 12px">
							<p style="font-weight: 700; font-size: 0.905rem; margin: 0">{{ finding.title }}</p>
							<p v-if="finding.mentioned_table_count" class="rc-muted" style="font-size: 0.78rem; margin: 1px 0">
								{{ t('recorder.report.mentionedAt', { count: finding.mentioned_table_count }, finding.mentioned_table_count) }}
							</p>
							<p style="font-size: 0.875rem; margin: 3px 0 0">{{ finding.summary }}</p>
						</div>
					</template>
				...
				<p class="rc-muted" style="font-size: 0.75rem; line-height: 1.5">
